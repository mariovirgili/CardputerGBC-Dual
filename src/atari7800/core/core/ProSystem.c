...
PROSYS_HOT void prosystem_ExecuteFrame(const uint8_t* input)
{
...
bool prosystem_Save(char *buffer, bool fast_saves)
{
   ...
   for(index = 0; index < 16384; index++)
      buffer[size + index] = mem_rd((uint16_t)index);
   size += 16384;
   ...
   {
      for(index = 0; index < 16384; index++)
         buffer[size + index] = mem_rd((uint16_t)(16384 + index));
      size += 16384;
   }
   else if(cartridge_type == CARTRIDGE_TYPE_SOUPER)
   {
      ...
      for(index = 0; index < MEMORY_SOUPER_EXRAM_SIZE; index++)
         buffer[size + index] = memory_souper_ram[index];
      size += MEMORY_SOUPER_EXRAM_SIZE;
...
bool prosystem_Load(const char *buffer, bool fast_saves)
{
   ...
   for(index = 0; index < 16384; index++)
      mem_wr((uint16_t)index, buffer[offset + index]);
   offset += 16384;
   ...
   {
      for(index = 0; index < 16384; index++)
         mem_wr((uint16_t)(16384 + index), buffer[offset + index]);
      offset += 16384;
   }
   else if(cartridge_type == CARTRIDGE_TYPE_SOUPER)
   {
