...
/* On ESP32, replace every memory_Read() in this file with the fast
 * inline version that avoids the cross-file function call, the
 * memory_IsReady() check, and the switch statement.  This is the
 * single biggest hot-path optimisation for the 6502 core. */
#ifdef ESP_PLATFORM
#include "esp_attr.h"
#define memory_Read(addr) memory_ReadFast(addr)
/* Put the entire 6502 executor in IRAM to eliminate instruction-cache
 * misses on the 256-opcode dispatch loop.  The compiled function is
 * ~12–18 KB; ESP32-S3 has ≥80 KB of user IRAM available. */
/* IRAM_ATTR intentionally NOT applied to sally_ExecuteInstruction: the
 * 256-opcode switch table's rodata (~121 KB) would be copied from flash
 * (DROM) into DRAM, exhausting the heap before paged memory can allocate.
 * Leave Sally running from flash cache; only MARIA hot paths get IRAM. */
#define SALLY_HOT
#else
#define SALLY_HOT
#endif
...
static void sally_BRK(void)
{
   ...
   sally_pc.b.l = mem_rd(SALLY_IRQ.L);
   sally_pc.b.h = mem_rd(SALLY_IRQ.H);
}
...
SALLY_HOT uint32_t sally_ExecuteInstruction(void)
{
...
SALLY_HOT uint32_t sally_ExecuteRES(void)
{
   sally_p      = SALLY_FLAG.I | SALLY_FLAG.R | SALLY_FLAG.Z;
   sally_pc.b.l = mem_rd(SALLY_RES.L);
   sally_pc.b.h = mem_rd(SALLY_RES.H);
   return 6;
}

SALLY_HOT uint32_t sally_ExecuteNMI(void)
{
   sally_Push(sally_pc.b.h);
   sally_Push(sally_pc.b.l);
   sally_p &= ~SALLY_FLAG.B;
   sally_Push(sally_p);
   sally_p |= SALLY_FLAG.I;
   sally_pc.b.l = mem_rd(SALLY_NMI.L);
   sally_pc.b.h = mem_rd(SALLY_NMI.H);
   return 7;
}

SALLY_HOT uint32_t sally_ExecuteIRQ(void)
{
