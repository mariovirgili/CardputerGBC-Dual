...
static void* libretro_Alloc(size_t size, bool preferInternal)
{
#ifdef ESP_PLATFORM
   void* ptr = NULL;
...
{
   if(!pokeyMixBuffer)
      pokeyMixBuffer = (uint8_t*)libretro_Alloc(AUDIO_SAMPLE_BUFFER_SIZE * sizeof(uint8_t), true);

   if(!audioOutBuffer)
      audioOutBuffer = (int16_t*)libretro_Alloc((AUDIO_SAMPLE_BUFFER_SIZE << 1) * sizeof(int16_t), true);

   return pokeyMixBuffer != NULL && audioOutBuffer != NULL;
}
...
static void sound_Store(void)
{
   ...
   /* Defer audio buffer allocation until the game is already running so
    * MARIA and main RAM get first access to the fragmented internal heap. */
   if((!pokeyMixBuffer || !audioOutBuffer) && !libretro_EnsureBuffers())
      return;
...
bool retro_load_game(const struct retro_game_info *info)
{
   ...
   /* The launcher owns the ROM buffer and keeps it valid for the whole session. */
   ...
   if(!libretro_EnsureBuffers())
      printf("[A7800][CORE] audio buffers unavailable, running muted until heap recovers\n");
...
void *retro_get_memory_data(unsigned id)
{
    if ( id == RETRO_MEMORY_SYSTEM_RAM )
        return memory_ram;  /* lower half on ESP, full on desktop */
    return NULL;
}
...
void retro_run(void)
{
   ...
   videoHeight = maria_surface_height ? maria_surface_height : Rect_GetHeight(&maria_visibleArea);
   buffer      = maria_surface_rows;

   video_cb(buffer, videoWidth, videoHeight, 0);
