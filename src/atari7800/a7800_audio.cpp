...
static void a7800_audio_task(void* arg)
{
    ...
    while (s_running) {
        ...
        const int n = a7800_ring_pop(local, kChunkSamples);
        if (n > 0) {
            cardputer_audio::queueRuntimeAudioBuffer(
                s_playBuf, s_playSlot, (size_t)n, s_sampleRate, false, kChannel);
        } else {
            vTaskDelay(pdMS_TO_TICKS(1));
        }
    }
...
void a7800_audio_init(unsigned sampleRate)
{
    ...
    cardputer_audio::beginSpeaker(s_sampleRate, false, 512, 8, 80, "a7800", 4, 0);
    M5Cardputer.Speaker.stop(kChannel);
    cardputer_audio::allocRuntimeAudioBuffers(s_playBuf, kChunkSamples, "a7800");
    s_playSlot = 0;
