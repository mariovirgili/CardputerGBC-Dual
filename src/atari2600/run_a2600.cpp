...
void run_a2600(const uint8_t* romData, size_t romLen, const char* romName)
{
...

    {
        ...
        display.initialize();
        if (useExternal) {
            display.topBar("A2600 ON EXTERNAL TFT", false, false);
            display.showControlBindings(
                share::emuControlActionLabels(share::EmuProfile::A2600),
                share::emuControlKeyLabels(share::EmuProfile::A2600),
                "GO / HOLD ESC = QUIT"
            );
        } else if (!emu_is_aux_screen_locked()) {
            a2600_display_show_external_info(romName);
        }
    }
