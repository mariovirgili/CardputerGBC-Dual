...
bool coleco_video_game_on_external(void)
{
    return g_emu_display_target == EMU_DISPLAY_EXTERNAL;
}

bool coleco_video_use_external_rgb444(void)
{
...
void coleco_video_init_palette(void)
{
    ...
    for (int i = 0; i < 16; ++i) {
        s_palette565[i] = coleco_rgb565(kPalette[i][0], kPalette[i][1], kPalette[i][2]);
    }
}

void coleco_video_reset_layout_cache(void)
{
    s_lastMode = -1;
    s_lastSrcW = -1;
    s_lastSrcH = -1;
...
void coleco_video_init_palette_pairs(const uint16_t* palette)
{
    if (!palette) {
        return;
    }
...
void coleco_video_expand_indexed_line(const uint8_t* src,
                                   uint16_t* dst,
                                   int pixelCount,
...
void coleco_video_compute_plan(unsigned srcW, unsigned srcH, ColecoVideoPlan* plan)
{
    if (!plan) {
        return;
    }
    ...
    const ColecoInternalViewMode mode = coleco_config_get_active_view_mode();
    const int targetW = coleco_video_target_w();
    const int targetH = coleco_video_target_h();
    if (mode == ColecoInternalViewMode::PixelPerfect) {
...
bool coleco_video_layout_changed(const ColecoVideoPlan& plan, unsigned srcW, unsigned srcH)
{
    const int mode = static_cast<int>(coleco_config_get_active_view_mode());
    const bool changed =
        s_lastMode != mode ||
...
void coleco_video_pack_rgb444_line(const uint16_t* src, int pixelCount, uint8_t* dst)
{
...
void coleco_video_begin_active_write(const ColecoVideoPlan& plan)
{
    if (coleco_video_game_on_external()) {
        coleco_video_prepare_external_tft();
        s_extTft.startWrite();
...
void coleco_video_end_active_write(void)
{
    if (coleco_video_game_on_external()) {
        s_extTft.endWrite();
    } else {
...
void coleco_video_draw_crop_frame(const ColecoDisplayFrame* frame, const ColecoVideoPlan& plan)
{
...
void coleco_video_draw_scaled_frame(const ColecoDisplayFrame* frame, const ColecoVideoPlan& plan)
{
    const uint16_t* palette = frame->palette565 ? frame->palette565 : s_palette565;
    const bool useRgb444 = coleco_video_use_external_rgb444();
    coleco_video_init_palette_pairs(palette);
...
bool coleco_video_render_frame_now(const ColecoDisplayFrame* frame)
{
    ...
    coleco_video_compute_plan(frame->width, frame->height, &plan);
    const bool layoutChanged = coleco_video_layout_changed(plan, frame->width, frame->height);
    ...
    if (plan.cropOnly) {
        coleco_video_draw_crop_frame(frame, plan);
    } else {
        coleco_video_draw_scaled_frame(frame, plan);
    }
...
{
    if (coleco_video_game_on_external() && !s_externalUiActive) {
...
bool coleco_video_present_frame(const ColecoDisplayFrame* frame)
{
    ...
    bool result = coleco_video_render_frame_now(frame);
