...
void C64Sys::checkJoystickOnlyStatemachine(bool jOMBpressed) {
  ...
  if (joystickOnlyModeState == JoystickOnlyModeState::NONE) {
    ...
    if (downpressed) {
      ...
      JoystickOnlyTextKeycode nextKeycode = getNextKeycode();
      actInGameKeycode = nextKeycode.keycode;
      ingamebox[47] = nextKeycode.text.dc00;
      ingamebox[48] = nextKeycode.text.dc01;
      ingamebox[49] = nextKeycode.text.modifier;
      vic.drawDOIBox(ingamebox, 12, 1, 15, 9, 1, 0, 65535, 1);
    } else if (rightpressed) {
      switch (joystickmode) {
      case 0:
