...
static uint8_t (*snapedlines)[256][2] = NULL;
...
bool o2em_machine_alloc_buffers(void)
{
   if (!rom_table)
   {
      rom_table = (uint8_t (*)[4096])calloc(VIDEOPAC_ROM_BANKS, sizeof(*rom_table));
      if (!rom_table)
         return false;
   }

#if defined(VIDEOPAC_ENABLE_SNAPLINES)
   if (!snapedlines)
   {
      snapedlines = (uint8_t (*)[256][2])calloc(MAXLINES + 2 * MAXSNAP, sizeof(*snapedlines));
      if (!snapedlines)
      {
         free(rom_table);
         rom_table = NULL;
...
void o2em_machine_free_buffers(void)
{
   free(snapedlines);
   snapedlines = NULL;
   free(rom_table);
...
void init_system(void)
{
   ...
   if (snapedlines)
      memset(snapedlines, 0, (MAXLINES + 2 * MAXSNAP) * sizeof(*snapedlines));
...
void init_roms(void)
{
   if (!rom_table && !o2em_machine_alloc_buffers())
      exit(EXIT_FAILURE);
...
void write_p1(uint8_t d)
{
   ...
   if (app_data.bank == 2)
      ...
      rom = rom_table[(p1 & 1) ? 0 : (romlatch % VIDEOPAC_ROM_BANKS)];
}
...
void ext_write(uint8_t dat, uint16_t adr)
{
   ...
   else if (!(p1 & 0x10) && !(p1 & 0x40))
   {
      ...
      else
      {
         if (app_data.bank == 4)
         {
            romlatch = ((~dat) & 7) % VIDEOPAC_ROM_BANKS;
