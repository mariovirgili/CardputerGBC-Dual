...
#if defined(VIDEOPAC_LOW_MEMORY_VIDEO) && (EMU_STATIC_POOL_SIZE < (BMPW * BMPH * 2u))
#error "Videopac low-memory VDC buffers do not fit in the shared emulator pool"
#endif
...
void o2em_vdc_get_palette565(uint16_t out_palette[256])
{
   ...
   if (!out_palette)
      return;
   for (i = 0; i < 256; ++i)
      out_palette[i] = RGB565(colors[i].r, colors[i].g, colors[i].b);
}
...
#if defined(VIDEOPAC_LOW_MEMORY_VIDEO)
	int y = ypos >> 1;
	unsigned int pnt = y * BMPW + (xpos - 8) + BORDERW;
#else
	int y=(ypos & 0xFE); 
	unsigned int pnt = y * BMPW + ((xpos-8) * 2) + BORDERW;
#endif
...
#endif
      {
         for (j=0; j<n; j++)
         {
            ...
            for (b=0; b<8; b++)
            {
               ...
               {
#if defined(VIDEOPAC_LOW_MEMORY_VIDEO)
                  if ((xpos - 8 + b < WNDW) && (y + j < BMPH))
                     mputvid(pnt, 1, cl, COL_CHAR);
#else
...
static void draw_quad(uint8_t ypos, uint8_t xpos, uint8_t cp0l, uint8_t cp0h, uint8_t cp1l, uint8_t cp1h, uint8_t cp2l, uint8_t cp2h, uint8_t cp3l, uint8_t cp3h)
{
	...
	pnt = (ypos >> 1) * BMPW + (xpos - 8) + BORDERW;
	...
	if (pnt + BMPW * lines < (unsigned long) clip_low) return;

	qcol[0] = (cp0h & 0xe) >> 1;
	qcol[1] = (cp1h & 0xe) >> 1;
	qcol[2] = (cp2h & 0xe) >> 1;
	qcol[3] = (cp3h & 0xe) >> 1;
	for(i = 0; i < 4; i++) qcol[i] = ((qcol[i] & 2) | ((qcol[i] & 1) << 2) | ((qcol[i] & 4) >> 2)) + 8;

	while(lines-- > 0)
	{
		off = 0;
		for(i = 0; i < 4; i++) {
			uint8_t bits = cset[chp[i]];
			for(j = 0; j < 8; j++) {
				if((bits & (1 << (7-j))) && (off < BMPW)) {
					mputvid(pnt + off, 1, qcol[i], COL_CHAR);
				}
				off += 1;
			}
			off += 8;
   ...
   if (VDCwrite[0xA0] & 0x40) {
      for(j=0; j<9; j++) {
         ...
         for (i=0; i<9; i++) {
            ...
            mputvid(pn1, 2, (color & 0x07) | ((color & 0x40) >> 3) | ((color & 0x80) ? 0 : 8), COL_HGRID);
            color = ColorVector[j*24+25];
            mputvid(pn1+BMPW, 2, (color & 0x07) | ((color & 0x40) >> 3) | ((color & 0x80) ? 0 : 8), COL_HGRID);
#if !defined(VIDEOPAC_LOW_MEMORY_VIDEO)
        ...
        if ((x<164) && (y>0) && (y<232)) {
			...
			if (t & 4) {
				...
				if ((pnt + BMPW * 16 >= (unsigned long)clip_low) && (pnt <= (unsigned long)clip_high)) {
					for (j=0; j<8; j++) {
						unsigned int row_pnt = pnt + (j * 2 * BMPW);
						sm = (((j%2==0) && (((t>>1) & 1) != (t & 1))) || ((j%2==1) && (t & 1))) ? 1 : 0;
						d1 = VDCwrite[pnt2++];
						for (b=0; b<8; b++) {
							if (d1 & 0x01) {
								if ((x + (b * 2) + sm < WNDW) && (y + (j * 2) + 1 < BMPH)) {
									mputvid(row_pnt + sm + (b * 2), 2, cl, c);
									mputvid(row_pnt + BMPW + sm + (b * 2), 2, cl, c);
								}
							}
							d1 = d1 >> 1;
						}
...
#if defined(VIDEOPAC_LOW_MEMORY_VIDEO)
				if ((pnt + BMPW * 8 >= (unsigned long)clip_low) && (pnt <= (unsigned long)clip_high)) {
					for (j=0; j<8; j++) {
						unsigned int row_pnt = pnt + (j * BMPW);
						sm = (((j%2==0) && (((t>>1) & 1) != (t & 1))) || ((j%2==1) && (t & 1))) ? 1 : 0;
						d1 = VDCwrite[pnt2++];
						for (b=0; b<8; b++) {
							if (d1 & 0x01) {
								if ((x+b+sm<WNDW) && (y+j<BMPH)) {
									mputvid(row_pnt+sm+b,1,cl,c);
								}
							}
							d1 = d1 >> 1;
						}
...
void init_display(void)
{
   create_cmap();
#if defined(VIDEOPAC_LOW_MEMORY_VIDEO)
   vscreen = g_emu_static_pool;
   col = g_emu_static_pool + (BMPW*BMPH);

   if ((!vscreen) || (!col)) {
      printf("[O2EM]: Could not allocate low-memory VDC buffers (%u bytes each).\n",
             (unsigned)(BMPW*BMPH));
      exit(EXIT_FAILURE);
   }
   printf("[O2EM]: Low-memory VDC buffers ready (%ux%u, %u bytes each).\n",
          (unsigned)BMPW,
          (unsigned)BMPH,
          (unsigned)(BMPW*BMPH));
