...
static const uint8_t *memory_bios_data = NULL;
static size_t memory_bios_size = 0;
...
static uint32_t o2em_crc32(uint32_t crc, const uint8_t *data, size_t size)
{
   crc = ~crc;
   while (size--)
   {
...
static bool ensure_video_buffer(void)
{
   ...
   mbmp = (uint16_t*)calloc(TEX_WIDTH * TEX_HEIGHT, sizeof(*mbmp));
   if (!mbmp)
   {
      if (log_cb)
         log_cb(RETRO_LOG_ERROR, "[O2EM]: Could not allocate video buffer.\n");
      return false;
   }
...
{
   memory_bios_data = data;
   memory_bios_size = size;
}
...
static bool load_bios(const char *biosname)
{
   ...
   if (!memory_bios_data || memory_bios_size < 1024)
   {
      log_cb(RETRO_LOG_ERROR, "[O2EM]: BIOS image missing or too small (%s).\n",
            biosname ? biosname : "memory");
      return false;
   }

   memcpy(rom_table[0], memory_bios_data, 1024);

   for (i=1; i<VIDEOPAC_ROM_BANKS; i++)
      memcpy(rom_table[i],rom_table[0],1024);

   crc = o2em_crc32(0, rom_table[0], 1024);
...
static bool load_cart(const uint8_t *data, size_t size)
{
   ...
   /* Get ROM CRC */
   app_data.crc = o2em_crc32(0, data, size);
       ...
       (size == 1048576))
   {
      ...
      log_cb(RETRO_LOG_ERROR, "[O2EM]: MegaCart content is not supported in low-memory mode.\n");
      return false;
#else
   ...
   else if (((size % 3072) == 0))
   {
      ...
      if (nb > VIDEOPAC_ROM_BANKS)
      {
         log_cb(RETRO_LOG_ERROR, "[O2EM]: ROM requires %d banks, low-memory build has %d.\n",
               nb, VIDEOPAC_ROM_BANKS);
         return false;
      }
...
static void upate_audio(void)
{
   ...
   if (low_pass_enabled)
   {
      ...
      for(i = 1; i <= SOUND_BUFFER_LEN; i++)
      {
         ...
         /* Get current sample */
         sample16  = ((((int)(*(audio_samples_ptr++)) - 128) *
               audio_volume) / 100) << 8;
...
bool retro_load_game(const struct retro_game_info *info)
{
   ...
   if (!o2em_machine_alloc_buffers())
   {
      log_cb(RETRO_LOG_ERROR, "[O2EM]: Could not allocate machine buffers.\n");
      return false;
   }

   if (!ensure_video_buffer())
      return false;

   if (!load_bios(bios_file_name))
      return false;
   ...
   if (app_data.voice)
   {
      ...
      snprintf(voice_path, sizeof(voice_path), "%s%s%s",
            system_directory_c ? system_directory_c : "",
            system_directory_c ? "/" : "",
            "voice");
...
void retro_init(void)
{
   ...
   low_pass_prev              = 0;
   RLOOP                      = 1;
   memset(soundBuffer, 0x80, sizeof(soundBuffer));
   memset(audioOutBuffer, 0, sizeof(audioOutBuffer));

   if (ensure_video_buffer() && mbmp)
      memset(mbmp, 0, TEX_WIDTH * TEX_HEIGHT * sizeof(*mbmp));
...
void retro_run(void)
{
   ...
   int frame_h = 0;
   int frame_pitch = 0;
   const uint8_t *indexed_frame = NULL;
#endif
...
#if defined(FRONTEND_SUPPORTS_INDEXED_VIDEO)
   indexed_frame = o2em_vdc_get_framebuffer(&frame_w, &frame_h, &frame_pitch);
   video_cb(indexed_frame, (unsigned)frame_w, (unsigned)frame_h, (size_t)frame_pitch);
#else
   if (crop_overscan)
   {
