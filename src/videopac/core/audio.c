...
void audio_process(unsigned char *buffer)
{
   ...
   while (pnt < SOUND_BUFFER_LEN)
   {
      ...
      if (enabled && volume)
      {
         int amp = AUDIO_STEP * volume;
         int bit = (aud_data & 0x01) ^ rndbit;
         buffer[pnt++] = (unsigned char)(AUDIO_SILENCE + (bit ? amp : -amp));
      }
      else
      {
         buffer[pnt++] = AUDIO_SILENCE;
      }
      cnt++;
