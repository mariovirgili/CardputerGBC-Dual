...
int StartMSX(void)
{
  ...
  J=FREE_SLOT;
  if(J>=0) { ROMType[J]=MAP_FMPAC;LoadCart("FMPAC.ROM",J); }
  ...
  if(!StateName)
  {
    ...
    I=ROMName[0]? strlen(ROMName[0]):0;
    J=ROMName[1]? strlen(ROMName[1]):0;
    J=J>I? J:I;
    J=J>4? J:4;
    J+=4;
    ...
    if(StateName=malloc(J))
    {
      ...
      /* Copy name */
      strcpy(StateName,ROMName[0]? ROMName[0]:ROMName[1]? ROMName[1]:"fMSX");
      /* Find extension */
      P=strrchr(StateName,'.');
...
void TrashMSX(void)
{
  ...
  for(J=0;J<MAXCARTS;J++)
    ...
    {
      if(Verbose) printf("Writing %s...",SRAMName[J]);
...
int LoadCart(const char *Name,int Slot)
{
  ...
  if((ROMType[Slot]>=MAP_GUESS)&&(ROMMask[Slot]+1>4))
  {
    ...
    if(Verbose) printf("guessed %s..",ROMNames[ROMType[Slot]]);
  }
  ...
  if(MAP_SRAM(ROMType[Slot]))
  {
    ...
    if(SRAMName[Slot]=malloc(strlen(Name)+5))
    {
      ...
      strcpy(SRAMName[Slot],Name);      
      if(P=strrchr(SRAMName[Slot],'.')) strcpy(P,".sav");
